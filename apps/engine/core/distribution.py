import logging
import time
import random
import os
from abc import ABC, abstractmethod
from typing import Dict, Any

logger = logging.getLogger(__name__)


class DistributionChannel(ABC):
    @abstractmethod
    def upload_content(self, content_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Uploads content to the external platform.
        Returns: {"status": "live"|"pending"|"failed", "platform_id": str, "url": str}
        """
        pass

    @abstractmethod
    def get_metrics(self, platform_id: str) -> Dict[str, float]:
        """Fetches real-time metrics (views, likes) from the platform."""
        pass


class MockTikTokChannel(DistributionChannel):
    """
    Simulates a high-fidelity TikTok API integration.
    State is instance-level so different users do not share mock data.
    """

    def __init__(self):
        # Instance-level state — no cross-user pollution
        self._mock_db: Dict[str, Dict] = {}

    def upload_content(self, content_data: Dict[str, Any]) -> Dict[str, Any]:
        if random.random() < 0.05:  # 5% simulated failure rate
            return {"status": "failed", "error": "API Timeout"}

        post_id = f"tt_{int(time.time())}_{random.randint(1000, 9999)}"
        self._mock_db[post_id] = {
            "views": 0,
            "likes": 0,
            "shares": 0,
            "created_at": time.time(),
        }
        logger.debug("[MOCK_TIKTOK] Uploaded post %s", post_id)
        return {
            "status": "live",
            "platform_id": post_id,
            "url": f"https://www.tiktok.com/@antigravity/video/{post_id}",
        }

    def get_metrics(self, platform_id: str) -> Dict[str, float]:
        return self._mock_db.get(platform_id, {"views": 0, "likes": 0, "shares": 0})

    def update_metrics(self, platform_id: str, new_views: int):
        if platform_id in self._mock_db:
            self._mock_db[platform_id]["views"] += new_views
            self._mock_db[platform_id]["likes"] += int(new_views * 0.1)
            self._mock_db[platform_id]["shares"] += int(new_views * 0.01)


class MockShopifyChannel(DistributionChannel):
    """Mock Shopify channel for development/testing."""

    def upload_content(self, content_data: Dict[str, Any]) -> Dict[str, Any]:
        product_id = f"shopify_{int(time.time())}_{random.randint(1000, 9999)}"
        logger.debug("[MOCK_SHOPIFY] Created product %s", product_id)
        return {
            "status": "live",
            "platform_id": product_id,
            "url": f"https://mock-store.myshopify.com/products/{product_id}",
        }

    def get_metrics(self, platform_id: str) -> Dict[str, float]:
        return {"views": 0, "orders": 0, "revenue": 0.0}


def get_tiktok_channel(db=None, user_id=None) -> DistributionChannel:
    """
    Factory: returns RealTikTokChannel if credentials exist, else MockTikTokChannel.
    Each call returns a fresh instance to avoid shared state between users.
    """
    from apps.engine.core.channels.tiktok import RealTikTokChannel
    from apps.engine.db.models import SocialConnection
    from apps.engine.core.security import decrypt_token

    token = os.getenv("TIKTOK_ACCESS_TOKEN")
    biz_id = os.getenv("TIKTOK_BUSINESS_ID")

    if db and user_id:
        conn = db.query(SocialConnection).filter(
            SocialConnection.user_id == user_id,
            SocialConnection.platform == "tiktok",
            SocialConnection.is_active == True,
        ).first()
        if conn and conn.access_token:
            token = decrypt_token(conn.access_token)
            biz_id = conn.account_id

    if token:
        logger.info("[DISTRIBUTION] Using real TikTok channel for user %s", user_id)
        return RealTikTokChannel(access_token=token, business_id=biz_id)

    logger.debug("[DISTRIBUTION] Using mock TikTok channel")
    return MockTikTokChannel()


def get_shopify_channel(db=None, user_id=None) -> DistributionChannel:
    """
    Factory: returns RealShopifyChannel if credentials exist, else MockShopifyChannel.
    """
    from apps.engine.core.channels.shopify import RealShopifyChannel
    from apps.engine.db.models import SocialConnection
    from apps.engine.core.security import decrypt_token

    token = os.getenv("SHOPIFY_ACCESS_TOKEN")
    shop_url = os.getenv("SHOPIFY_SHOP_URL")

    if db and user_id:
        conn = db.query(SocialConnection).filter(
            SocialConnection.user_id == user_id,
            SocialConnection.platform == "shopify",
            SocialConnection.is_active == True,
        ).first()
        if conn and conn.access_token:
            token = decrypt_token(conn.access_token)
            shop_url = conn.account_name

    if token:
        logger.info("[DISTRIBUTION] Using real Shopify channel for user %s", user_id)
        return RealShopifyChannel(shop_url=shop_url, access_token=token)

    logger.debug("[DISTRIBUTION] Using mock Shopify channel")
    return MockShopifyChannel()
